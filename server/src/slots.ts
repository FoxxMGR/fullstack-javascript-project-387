 ...
 *
 * Слоты генерируются по рабочему окну: Пн–Пт (кроме нерабочих праздничных
 * дней по ст. 112 ТК РФ) с 09:00 до 18:00 (локальное время сервера), шаг
 * сетки равен длительности типа события. Слот считается занятым, если
 * пересекается с любым существующим бронированием — владелец не может
 * находиться на двух встречах одновременно, даже разных типов.
 */
...
/** Является ли день рабочим (Пн–Пт и не праздничный). */
