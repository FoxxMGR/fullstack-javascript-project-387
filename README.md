...
`opencode` и `auto-review` из default-ветки и подставляют в `prompt`,
поэтому:
...
(`opencode`, `auto-review`, `night-check`) или сводный дашборд.
...
- **Права.** Права выданы по воркфлоу отдельно и минимальные: write на
  содержимое есть только у `opencode`, где агент реально создаёт
  коммиты/ветки/PR, авторевью — только чтение + `id-token`, ночной отчёт
  создаёт только issue (`issues: write`), без прав на содержимое и PR.
